...
void connectController(ChessView& view, IChessController ** cont, int argc, char * argv[])
{
	...
	int playerSetting = atoi(argv[1]);
	if (argc != 2 || playerSetting < 0 || playerSetting > 3) {
		cout << "USAGE: chess <int 0-3>\n0: Human vs. Human\n1: Human vs. Computer\n2: Computer vs. Human\n3: Computer vs. Computer" << endl;
	}

	// 2. Initialize controller. This is tricky because it's a double pointer.
	// Example:
	(*cont) = new ChessController(playerSetting);
...
///@code
///g++ -g -o main.o (option) -DLOG_LEVEL_HIDE_MASK=(G_LOG_LEVEL_DEBUG|G_LOG_LEVEL_INFO) main.cpp
///@endcode
